...
describe('main handler processing tagged releases', () => {
  ...
  it('throws an error if the github event tag does not conform to semantic versioning', async () => {
    ...
    await expect(main()).rejects.toThrow(
      'The parameter "last_release_tag" was not set and the current tag "faketag" does not appear to conform to semantic versioning.',
    )
  })
  ...
  it('should create a new release', async () => {
    ...
    nock('https://api.github.com')
      .matchHeader('authorization', `token ${testGhToken}`)
      .get(`/repos/marvinpinto/private-actions-tester/git/ref/tags%2Fv0.0.0`)
      .reply(404)
    ...
    expect(exportVariable).toHaveBeenCalledWith('AUTOMATIC_RELEASES_TAG', 'v0.0.1')
  })
})
