...
import { ReposCompareCommitsResponseCommitsItem } from '@octokit/rest'
...
const searchForPreviousReleaseTag = async (
  ...
  if (!validSemver) {
    throw new Error(
      `The parameter "last_release_tag" was not set and the current tag "${currentReleaseTag}" does not appear to conform to semantic versioning.`,
    )
  }
...
const getCommitsSinceRelease = async (
  ...
  await client.git
    .getRef(tagInfo)
    .then(() => {
      core.info(`Found SHA corresponding to tag "${tagInfo.ref}"`)
  ...
  let secondResp: Octokit.Response<Octokit.ReposCompareCommitsResponse>
  try {
    secondResp = await client.repos.compareCommits({
      owner: tagInfo.owner,
      repo: tagInfo.repo,
    ...
    core.info(
      `Successfully retrieved ${secondResp.data.commits.length} commits between ${previousReleaseRef} and ${currentSha}`,
    )
  } catch (_err) {
    core.warning(`Could not find any commits between ${previousReleaseRef} and ${currentSha}`)
  }

  let commits: Array<ReposCompareCommitsResponseCommitsItem> = []
  if (secondResp?.data?.commits) {
    commits = secondResp.data.commits
  }
  core.debug(`Currently ${commits.length} number of commits between ${previousReleaseRef} and ${currentSha}`)

  core.info(`${ignoreAuthors}`)

  commits = commits.filter((commit) => {
    core.debug(`Checking commit: ${commit.author.login}`)
    return commit.author ? !ignoreAuthors.includes(commit.author.login) : true
  })
...
export const run = async (inputs: Inputs): Promise<void> => {
  try {
    ...
    const client = new github.GitHub(inputs.repoToken, {
    ...
    const releaseTag = inputs.releaseVersion ? inputs.releaseVersion : parseGitTag(context.ref)
    if (!releaseTag) {
      throw new Error(
        `The parameter "release_version" was not set and this does not appear to be a GitHub tag event. (Event: ${context.ref})`,
      )
    }

    const previousReleaseTag = inputs.lastTagName
      ? inputs.lastTagName
    ...
    const releaseUploadUrl = await generateNewGitHubRelease(client, {
      ...
      repo: context.repo.repo,
      tag_name: releaseTag,
      name: inputs.releaseTitle ? inputs.releaseTitle : releaseTag,
      draft: inputs.draftRelease,
      prerelease: inputs.preRelease,
      body: changelog,
    })

    await uploadReleaseArtifacts(client, releaseUploadUrl, inputs.files)
